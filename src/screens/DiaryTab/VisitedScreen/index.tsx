...
const VisitedScreen = (props: ScreenProps) => {
  ...
  const visited: Visited | null = useSelector(
    (state: VisitedsStateType) => state.current,
  );
  const visitedId: number = routeParam(props.route, '_id');
  const [title, setTitle] = useState<string>(visited?.title ?? '');
  // const [state, setState] = useState<boolean>(true);
  const [pre, setPre] = useState<number | null | undefined>(visited?.pre);
  ...
  useEffect(() => {
    if (visitedId !== undefined) {
      dispatch(visitedsAction.getVisited({_id: visitedId}));
    }
  }, []);
  useEffect(() => {
