...
import { COLORS, DIMENS, FONT_SIZE, IMG_DEFAULT_AVATAR, STORAGE_KEY, STRINGS } from '../../../common';
...
const Profile = (prop: ViewProps) => {
  ...
  const { isAuthorized } = useSelector((state: RootStateType) => state.healthState)
  const RBSheetRefAccount = useRef<any>()
  const RBSheetRefHMM = useRef<any>()
  const { avatar, username } = useSelector((state: RootStateType) => state.userState.customInfor)
  ...
  return (
    <ContainerView style={{ backgroundColor: COLORS.BACKGROUND }}>
      <ScrollView style={styles.container} contentContainerStyle={{ marginHorizontal: 8 }}>
        <Section>
          <View style={{ alignItems: 'center' }}>
            <TouchableOpacity style={{ alignItems: 'center' }} onPress={gotoProfileScreen}>
              <Avatar.Image style={{
                ...
                shadowOpacity: 0.3,
                elevation: 5
              }} source={(avatar == "" || avatar == null) ? IMG_DEFAULT_AVATAR : { uri: avatar }} size={72} />
              <Text style={{ fontWeight: 'bold', fontSize: 18, color: COLORS.BLACK, marginTop: 4 }}>{username}</Text>
            </TouchableOpacity>
          </View>
        </Section>
        <Section title='Danh sách'>
          <FrameButton buttons={[{ title: "Các lần khám", onPress: listVisited }, { title: "Các thuốc đã uống", onPress: listMedicine }]} />
        </Section>
        ...
        <Section title='Thêm'>
          <FrameButton buttons={[{ title: "Đặt mục tiêu", onPress: () => { } }, { title: "Thay đổi phương thức theo dõi sức khỏe", onPress: () => { RBSheetRefHMM.current.open() } }, {
      ...
      <RBSheet ref={RBSheetRefAccount} height={2 * DIMENS.BUTTON_RBSHEET_HEIGHT} openDuration={250}>
      ...
      <RBSheet ref={RBSheetRefHMM} openDuration={250} height={2 * DIMENS.BUTTON_RBSHEET_HEIGHT} >
        <TouchableOpacity style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }} onPress={chooseGF}>
          <Text style={{ color: COLORS.BLACK, fontSize: FONT_SIZE.HEADER_TAG }}>Thiết bị này</Text>
        </TouchableOpacity>
        <HairLine style={{ width: '90%', backgroundColor: 'black' }} />
        <TouchableOpacity style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }} onPress={chooseOtherDevice}>
          <Text style={{ color: COLORS.BLACK, fontSize: FONT_SIZE.HEADER_TAG }}>Thiết bị ngoại vi</Text>
        </TouchableOpacity>
      </RBSheet>
    </ContainerView>
  );
